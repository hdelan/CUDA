 ...
 */
/* ----------------------------------------------------------------------------*/
__global__ void sum_abs_rows_GPU(double * matrix_GPU, double * rowsum_GPU, const int N, const int M) {
 ...
 */
/* ----------------------------------------------------------------------------*/
__global__ void sum_abs_cols_GPU(double * matrix_GPU, double * colsum_GPU, const int N, const int M) {
        int idx = blockIdx.x*blockDim.x + threadIdx.x;
        if (idx < M) {
        	colsum_GPU[idx] = 0.0f;
                for (int j = 0; j < N; ++j) {
...
double vector_reduction_GPU(double * vector_GPU, const int N, dim3 dimBlock, dim3 dimGrid) {
	...
	double ans;
	cudaMemcpy(&ans, vector_GPU, sizeof(double), cudaMemcpyDeviceToHost);
	return ans;
}
