...
void GPU_rowsum(dim3 dimGrid_row, dim3 dimBlock, float * A_d, float * rowsum_d, int N, int M){
	sum_abs_rows_GPU<<<dimGrid_row, dimBlock>>>(A_d, rowsum_d, N, M);
}

void GPU_colsum(dim3 dimGrid_col, dim3 dimBlock, float * A_d, float * colsum_d, int N, int M){
...
*/
  ...
  GPU_rowsum(dimGrid_row, dimBlock, A_d, rowsum_d, N, M);
  //sum_abs_rows_GPU<<<dimGrid_row, dimBlock>>>(A_d, rowsum_d, N, M);
  ...
  GPU_colsum(dimGrid_col, dimBlock, A_d, colsum_d, N, M);
  //sum_abs_cols_GPU<<<dimGrid_col, dimBlock>>>(A_d, colsum_d, N, M);
  ...
  free(colsum);
  colsum = (float *) calloc(N, sizeof(float));  
